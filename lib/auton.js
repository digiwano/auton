var util    = require('util');
var _       = require('underscore');
var EE2     = require('eventemitter2').EventEmitter2;
var wrench  = require('wrench');
var UI      = require('./ui');
var Runner  = require('./runner');
var Watcher = require('./watcher');
var MW      = require('./middleware');
var _plugin = require('./plugin');
var VERSION = require("../package.json").version;
var path    = require('path');
var async   = require('async');

var _slice = Array.prototype.slice;

var defaultOptions = {};
var optionNames = ['root'];

function Auton( opts ) {
  var self = this;
  var o    = _.extend( {}, defaultOptions, opts );
  _.extend( this, _.pick(o, optionNames) );


  var _e          = process.env;
  this.env        = o.env || _e.AUTON_ENV || _e.NODE_ENV || 'development';
  this.options    = o;
  this.root       = o.root;
  this.started    = null;
  this.rules      = [];

  var ui          = new UI({ verbosity: o.cliVerbosity })

  this.plugin( 'ui', ui );
  this.success( 'Auton v' + VERSION + ' initialized in ' + this.env + ' mode' );

  var _r          = {root: this.root, ui: ui };
  var _wo         = _.extend( {}, _r, o.watcherOptions );
  var _ro         = _.extend( {}, _r, o.runnerOptions  );
  var watcher     = new Watcher( _wo );
  var runner      = new Runner(  _ro );
  this.plugin( 'watcher', watcher );
  this.plugin( 'runner',  runner );

  if (o.watch) {
    this.watch( o.watch );
  }
};

util.inherits( Auton, EE2 );
_.extend( Auton.prototype, _plugin );

Auton.Emitter    = EE2;
Auton.Middleware = MW;

Auton.prototype.plugin = function( pluginName, pluginInstance ) {
  var self = this;
  var p    = this[pluginName] = pluginInstance;
  p.on( 'trigger', function( file, stat ) {
    self.trigger( file, null, pluginName );
  } );
  p.on( 'delete', function(file, x, stat) {
    self.del( file, null, pluginName );
  });
  p.on( 'log',   this.log  .bind( this, pluginName ) );
  p.on( 'error', this.error.bind( this, pluginName ) );
};

Auton.prototype.scanAll = function(cb) {
  var self = this;
  cb = cb || function(){};

  self.info("Scanning all files!");
  async.forEachSeries(this.watcher.paths, function(p, next) {
    self.debug("scanning "+path.resolve(this.root, p));
    wrench.readdirRecursive( p, function(err, files) {
      if (  err   ) { return next(err); }
      if (! files ) { return next(); }

      files.forEach(function(f){ f = path.join(p,f); self.trigger(f); });
    });
  }, function(err) {
    if (err) {
      self.error(err);
      return cb(err);
    }
    self.info("all files scanned!");
    cb();
  });
};

Auton.prototype.error = function( source, msg ) {
  if (arguments.length === 1) {
    return this.log( 'core', 'error', msg );
  }
  this.log( source, 'error', msg );
};

Auton.prototype._preStartError = function(name) {
  throw new Error("."+name+"() must be called before .start()!");
};

Auton.prototype.del = function() {
  // file delete event. not yet implemented.
  // console.log("file deleted?", _slice.call(arguments));
};

// handles watch("foo"), watch("foo","bar"), and watch(["foo", "bar"])
Auton.prototype.watch = function watch( file ) {
  if (this.started) {
    return this._preStartError();
  }
  var self = this;
  var w    = (arguments.length === 1) ? [].concat( file ) : _slice.call( arguments );
  w.forEach(function(_w) {
    self.watcher.add( _w );
  });
};

Auton.prototype.trigger = function( filepath, cb, source ) {
  var self = this;
  return this.runner.run( filepath, function(err) {
    if (err) {
      self.error( 'file:'+filepath, err );
      return cb && cb( err );
    }
  }, source || 'core' );
};

Auton.prototype.configure = function( env, conf ) {
  if (arguments.length === 1   && typeof  env === 'function') { return  env(this); }
  if (this.env         === env && typeof conf === 'function') { return conf(this); }
};

Auton.prototype.use = function( pattern, fn ) {
  var self = this;
  if (arguments.length === 1) {
    fn      = pattern;
    pattern = true;
  }
  var ary = _.flatten( _slice.call(arguments, 1) );
  ary.forEach(function(mw) {
    self.runner.addMiddleware( pattern, mw );
  });
};

Auton.prototype.match = function( pattern, middleware,/* middleware, middleware, middleware, ...*/ rule ) {
  this.debug("adding rule for pattern: "+pattern.toString());
  var a   = _slice.call( arguments );
  pattern = a.shift();
  var mw  = _.flatten( a );
  this.runner.addRule( pattern, mw );
};

module.exports.Auton = Auton;