...
Runner.prototype.addRule = function( pattern, rules ) {
  this.rules.push( {pattern: pattern, mw: rules });
};

Runner.prototype.addMiddleware = function( pattern, fn ) {
  this.middleware.push( {pattern: pattern, fn: fn });
};

Runner.prototype.run = function( filepath, cb ) {
  this.debug('running ' + filepath );
  cb = cb || function(){};
  var self  = this;
  var noop  = function(file, next) { return next(); };

  async.forEachSeries(this.rules, function iter( ruleSet, next) {
    dispatchRule(self, filepath, ruleSet, next);
  }, function end(err) {
...
function dispatchRule( runner, filepath, ruleSet, callback ) {
  var file = new File( runner.root, runner.ui, filepath ); // each rule gets its own file instance
  file.on('log', runner.log.bind(runner) );
  ...
  var mwlist = [].concat( runner.middleware ).filter( _matches ).map( _getfn ).concat( ruleSet.mw );
